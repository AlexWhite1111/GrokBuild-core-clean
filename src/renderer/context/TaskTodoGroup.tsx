...
function TodoItem({ item, group }: { item: TodoEntrySnapshot; group: TodoGroupSnapshot }) {
  const status = effectiveStatus(item.status, group);
...
function effectiveStatus(status: TodoEntrySnapshot["status"], group: TodoGroupSnapshot): TodoEntrySnapshot["status"] {
  if (status === "completed" || status === "failed") return status;
  return group.endReason === "cancelled" || group.endReason === "interrupted" ? "cancelled" : status;
}
