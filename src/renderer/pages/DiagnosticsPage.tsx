...
export function DiagnosticsPage() {
  ...
  return <main className={styles.page}><header><div><Text as="h1" font="heading" size="title" weight="semibold">{t("diagnosticsLabel")}</Text><Text as="p" tone="muted" size="label">{t("diagnosticsSubtitle")}</Text></div><div><Control recipe="quiet" onClick={() => void window.grokDesktop?.openTerminal()}><ExternalLink size={13} />{t("openTerminalProject")}</Control><Control recipe="quiet" onClick={() => void refresh()}><RefreshCw size={13} />{t("reprobe")}</Control></div></header>
    ...
    <Surface as="section" appearance="surface" elevation="content" className={styles.panel}><h2>{t("typedRegistry")}</h2><div className={styles.table}><div className={styles.tableHead}><span>{t("method")}</span><span>{t("kind")}</span><span>{t("sideEffect")}</span><span>{t("status")}</span></div>{capabilities.acp.xai.map((method) => <div key={method.method}><code>{method.method}</code><span>{method.kind}</span><span>{method.sideEffect}</span><Text tone={method.availability === "advertised" || method.availability === "probed" ? "success" : "muted"} size="caption">{method.availability}{method.reason ? ` · ${method.reason}` : ""}</Text></div>)}</div></Surface>
