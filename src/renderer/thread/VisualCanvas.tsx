...
import { constrainVisualOffset } from "./VisualCanvasGeometry.js";
import styles from "./VisualCanvas.module.css";

type VisualSizeMode = UiPreferences["mediaInitialSize"] | "fit" | "double";
interface VisualView { mode: VisualSizeMode; zoom: number; x: number; y: number }
interface VisualLayout { width: number; height: number; paddingX: number; paddingY: number }
...
export function useVisualCanvasController(initialMode: VisualSizeMode = "native"): VisualCanvasController {
  ...
  return useMemo(() => ({
    ...
    fit: () => setView({ mode: "fit", zoom: 1, x: 0, y: 0 }),
  }), [view]);
}
...
export function VisualCanvas({ children, controller, naturalWidth, naturalHeight, ariaLabel, detail = false, comfortablePercent = 70, minimumSize = 64, className = "", contentClassName = "" }: {
  ...
  const [layout, setLayout] = useState<VisualLayout>({ width: 0, height: 0, paddingX: 0, paddingY: 0 });
  ...
  const viewportHeight = detail ? Math.max(1, layout.height - layout.paddingY) : Number.MAX_SAFE_INTEGER;
  ...
  const nativeScale = clamp(1, minimumScale, fitScale);
  ...
  const modeScale = controller.view.mode === "native" ? nativeScale
  ...
  const stageHeight = detail ? undefined : renderedHeight + layout.paddingY;
  const pannable = renderedWidth > 0 && renderedHeight > 0;

  const constrain = useCallback((x: number, y: number) => {
    const height = detail ? viewportHeight : Math.max(1, (stageHeight || 0) - layout.paddingY);
    return constrainVisualOffset({ x, y, renderedWidth, renderedHeight, viewportWidth, viewportHeight: height });
  }, [detail, layout.paddingY, renderedHeight, renderedWidth, stageHeight, viewportHeight, viewportWidth]);
  ...
  const measure = useCallback(() => {
    ...
    setLayout((current) => current.width === next.width && current.height === next.height && current.paddingX === next.paddingX && current.paddingY === next.paddingY ? current : next);
  }, []);
  ...
  const pointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    drag.current = { pointerId: event.pointerId, clientX: event.clientX, clientY: event.clientY, x: controller.view.x, y: controller.view.y };
  };
  const pointerMove = (event: PointerEvent<HTMLDivElement>) => {
    const origin = drag.current;
    if (!origin || origin.pointerId !== event.pointerId) return;
    event.preventDefault();
    const next = constrain(origin.x + event.clientX - origin.clientX, origin.y + event.clientY - origin.clientY);
  ...
  const wheel = (event: WheelEvent<HTMLDivElement>) => {
    if (event.ctrlKey) {
      ...
      const ratio = nextZoom / controller.view.zoom;
      const nextRenderedWidth = natural.width * modeScale * nextZoom;
      const nextRenderedHeight = natural.height * modeScale * nextZoom;
      const nextStageHeight = detail ? undefined : nextRenderedHeight + layout.paddingY;
      const visibleHeight = detail ? viewportHeight : Math.max(1, (nextStageHeight || 0) - layout.paddingY);
      const x = anchorX - (anchorX - controller.view.x) * ratio;
      const y = anchorY - (anchorY - controller.view.y) * ratio;
      const next = constrainVisualOffset({ x, y, renderedWidth: nextRenderedWidth, renderedHeight: nextRenderedHeight, viewportWidth, viewportHeight: visibleHeight });
      controller.setView((current) => ({ ...current, zoom: nextZoom, ...next }));
      return;
    }
  ...
  const cycleSize = () => {
    ...
    const next = nextMediaSizePreset(longEdge * scale, [
