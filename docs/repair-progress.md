...
- 页面滚动的唯一行为状态是“跟随最新”阅读意图：默认及真实末尾保持跟随，主动向上滚动释放；任务追加、流式行、Mermaid/图片高度和 Composer inset 变化只更新几何，不得把布局漂移误判为用户意图。
...
- 当前官方 Session 四份真实混排回复逐字重放：长鑫深度长文 4 / 4、多图轮 20 / 20、图片交替轮 11 / 11、图片 + 嵌套链接压力轮 3 / 3 个 Mermaid 均在闭合后的下一字符封存。
- 图片 + 嵌套链接压力轮为 6,915 字符、150 次更新：纯文本长尾修复后解析放大从 38.71 倍降至 4.03 倍，增长阶段从约 1,000.28 ms 降至约 67.29 ms；3 / 3 个既有段通过最终 canonical 校验保留。
